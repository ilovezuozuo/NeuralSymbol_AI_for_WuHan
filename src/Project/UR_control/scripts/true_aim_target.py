...
class TrueAimTarget(TrueBase):
    def get_tgt_pose_in_world_frame(self,all_info):
        ...
        # tgt_pose_in_real_frame.position.y = -self.y_shift
        tgt_pose_in_real_frame.position.z = -self.z_shift - 0.08
    ...
    def adjust_pose_in_world_frame(self):
        ...
        tgt_pose_in_effector_frame.position.x = 0.05*(random.random()-0.5)
        tgt_pose_in_effector_frame.position.y = 0.05*(random.random()-0.5)
        tgt_pose_in_effector_frame.position.z = 0.05*(random.random()-0.5)
        q = tf.transformations.quaternion_from_euler(0, 0, 0)
    ...
    def set_global_position(self):
        ...
        global_position.position.x = 0.38
        global_position.position.y = 0.58
        global_position.position.z = -0.25
    ...
    def action(self, all_info, pre_result_dict,kalman,yolo,bolt_is_dis):
        ...
        while not kalman.finished:
            ...
            if detect_ret:
                ...
                if (s==0):
                    ...
                    planner.curr_ob_pose=[real_pose.position.x, real_pose.position.y, real_pose.position.z, real_pose.orientation.x, real_pose.orientation.y, real_pose.orientation.z, real_pose.orientation.w]
                    print(real_pose)
                    ...
                    if  np_collected==False and (not temp_diff_list==[]):
                        ...
                        coarse_pose.position.x = coarse_pose_list[temp_diff_list.index(min(temp_diff_list))].position.x + 0.05
                        coarse_pose.position.y = coarse_pose_list[temp_diff_list.index(min(temp_diff_list))].position.y + 0.05
                        coarse_pose.position.z = -0.35
                        ...
                        planner.next_ob_pose=[coarse_pose.position.x, coarse_pose.position.y, coarse_pose.position.z, coarse_pose.orientation.x, coarse_pose.orientation.y, coarse_pose.orientation.z, coarse_pose.orientation.w]
                    ...
                    if  np_collected==False and (temp_diff_list==[]):
                        curr_pose= self.group.get_current_pose(self.effector).pose
                        try_new_pose=curr_pose
                        try_new_pose.position.x+=0.02
                        try_new_pose.position.y+=0.02
                        try_new_pose.position.z=-0.35
        ...
        if not real_pose is None:
            ...
            print(r,p,y)
            # rospy.sleep(1000)
            # target_pose = copy.deepcopy (real_pose)
            # # target_pose.position.y = target_pose.position.y + 0.06
            # target_pose.position.z = target_pose.position.z + 0.05
            # self.set_arm_pose(self.group, target_pose, self.effector)
